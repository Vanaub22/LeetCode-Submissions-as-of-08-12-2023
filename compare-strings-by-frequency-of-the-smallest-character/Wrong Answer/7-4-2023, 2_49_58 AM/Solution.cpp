...
public:
    ...
    vector<int> numSmallerByFrequency(vector<string>& queries, vector<string>& words) {
        ...
        for(string query:queries) fqueries.emplace_back(smallFreq(query));
        for(string word:words) fwords.emplace_back(smallFreq(word));
        for(int fquery:fqueries){ 
            ...
            answer.emplace_back(greaterWords);
        }
        return answer;
    }
};