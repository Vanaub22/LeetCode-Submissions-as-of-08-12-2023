...
public:
    ...
    void findMoreTrees(TreeNode* root, set<int>& to_delete, vector<TreeNode*>& forest) {
        ...
        if(to_delete.find(root->val)!=to_delete.end()) {
            if(root->left && to_delete.find(root->left->val)==to_delete.end()) forest.push_back(root->left);
            if(root->right && to_delete.find(root->right->val)==to_delete.end()) forest.push_back(root->right);
        }
